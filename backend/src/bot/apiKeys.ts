...
export function generateApiKey(
  botId: string,
  ownerId: string,
  permissions: ApiKeyPermission[] = ['play']
...
export function validateApiKey(rawKey: string): ApiKeyRecord | null {
  ...
  if (!key.startsWith('pgbot_')) return null
 ...
 */
export function getBotIdFromKey(rawKey: string): string | null {
  const record = validateApiKey(rawKey)
...
export function revokeApiKey(keyHash: string): boolean {
  ...
  if (deleted) persistApiKeysDebounced()
  return deleted
}
 ...
 */
export function revokeAllKeysForBot(botId: string): number {
  let count = 0
  ...
  if (count > 0) persistApiKeysDebounced()
  return count
}
 ...
 */
export function listKeysForBot(botId: string): ApiKeyRecord[] {
...
export function getApiKeyStats(): {
  ...
  for (const record of apiKeys.values()) {
    if (record.lastUsedAt > oneDayAgo) activeToday++
...
function persistApiKeysDebounced(): void {
...
async function persistApiKeys(): Promise<void> {
  try {
    ...
    const data: Record<string, ApiKeyRecord> = {}
  ...
  try {
    ...
    for (const [hash, record] of Object.entries(data)) {
      if (isValidRecord(record)) {
        apiKeys.set(hash, record as ApiKeyRecord)
      }
    }
...
function isValidRecord(obj: unknown): obj is ApiKeyRecord {
...
export function requireBotAuth(
  ...
  ;(req as any).botId = record.botId
  ;(req as any).botOwnerId = record.ownerId
  ;(req as any).botPermissions = record.permissions
...
export function requirePermission(permission: ApiKeyPermission) {
  return (req: Request, res: Response, next: NextFunction): void => {
    const permissions = (req as any).botPermissions as ApiKeyPermission[] | undefined
