...
const botNameIndex = new Map<string, string>() // name -> botId
...
export function registerBot(
  name: string,
  ownerId: string
): { bot: BotRegistration; apiKey: string } | { error: string } {
  // Validate name
 ...
 * Get a bot by ID
 */
export function getBot(botId: string): BotRegistration | null {
 ...
 * Get a bot by name
 */
export function getBotByName(name: string): BotRegistration | null {
...
export function updateBotStats(
  ...
  if (update.won) bot.stats.matchesWon++
 ...
 * Get ELO for a bot
 */
export function getBotElo(botId: string): number {
 ...
 * Set ELO for a bot
 */
export function setBotElo(botId: string, elo: number): void {
  const bot = bots.get(botId)
  if (bot) {
 ...
 */
export function createSession(botId: string): BotSession {
 ...
 * Get a session by ID
 */
export function getSession(sessionId: string): BotSession | null {
 ...
 */
export function getSessionByBotId(botId: string): BotSession | null {
 ...
 */
export function setSessionTable(
  sessionId: string, 
  tableId: string | null
): void {
  const session = sessions.get(sessionId)
 ...
 */
export function getConnectedBotCount(): number {
 ...
 * Get top bots by ELO
 */
export function getLeaderboard(limit: number = 50): Array<{
  rank: number
  botId: string
  name: string
  elo: number
...
function persistBotsDebounced(): void {
  if (persistTimer) clearTimeout(persistTimer)
  persistTimer = setTimeout(persistBots, 500)
}

async function persistBots(): Promise<void> {
  try {
    await fs.mkdir(DATA_DIR, { recursive: true })
    const data: Record<string, BotRegistration> = {}
...
export async function loadBots(): Promise<void> {
  try {
    ...
    for (const [botId, bot] of Object.entries(data)) {
      if (isValidBot(bot)) {
        bots.set(botId, bot as BotRegistration)
        botNameIndex.set((bot as BotRegistration).name.toLowerCase(), botId)
      }
    }
...
function isValidBot(obj: unknown): obj is BotRegistration {
