...
// In-memory store
const tournaments = new Map<string, ScheduledTournament>()
const tournamentResults = new Map<string, TournamentResult>()
...
export function createTournament(options: {
  ...
  const tournament: ScheduledTournament = {
    tournamentId,
    name: options.name,
...
export function scheduleDailyTournaments(): void {
  ...
  for (const { hour, name } of times) {
    ...
    const existing = Array.from(tournaments.values()).find(
      t => t.name === name && 
           t.status !== 'finished' && 
           t.status !== 'cancelled'
    )
 ...
 */
export function tickTournaments(): void {
  const now = Date.now()
 ...
 * Start a tournament
 */
function startTournament(tournament: ScheduledTournament): void {
 ...
 * Cancel a tournament
 */
function cancelTournament(tournament: ScheduledTournament, reason: string): void {
 ...
 */
function updateBlindLevel(tournament: ScheduledTournament): void {
  if (tournament.status !== 'running' && tournament.status !== 'final_table') return
  ...
  for (const level of tournament.blindStructure) {
    ...
    if (elapsed < totalMinutes * 60 * 1000) break
 ...
 */
function calculatePrizes(prizePool: number, playerCount: number): number[] {
  if (playerCount <= 2) {
    return [prizePool]
  }
 ...
 */
function calculateEloChange(place: number, totalPlayers: number): number {
 ...
 */
export function getTournament(tournamentId: string): ScheduledTournament | null {
 ...
 */
export function getUpcomingTournaments(): ScheduledTournament[] {
 ...
 */
export function getActiveTournaments(): ScheduledTournament[] {
  return Array.from(tournaments.values())
 ...
 */
export function getRecentResults(limit: number = 10): TournamentResult[] {
 ...
 */
export function getCurrentBlinds(tournamentId: string): BlindLevel | null {
  const tournament = tournaments.get(tournamentId)
...
function broadcastTournamentUpdate(tournament: ScheduledTournament): void {
...
function persistTournamentsDebounced(): void {
  ...
  try {
    ...
    if (data.tournaments) {
      for (const [id, t] of Object.entries(data.tournaments)) {
        tournaments.set(id, t as ScheduledTournament)
      }
    }
    
    if (data.results) {
      for (const [id, r] of Object.entries(data.results)) {
        tournamentResults.set(id, r as TournamentResult)
      }
    }
