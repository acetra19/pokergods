...
const ADMIN_JWT_SECRET = (() => {
    const existing = process.env.ADMIN_JWT_SECRET || global.__pg_admin_jwt;
    if (existing)
        return existing;
    const generated = randomBytes(32).toString('hex');
    global.__pg_admin_jwt = generated;
    return generated;
})();
...
app.post('/profile/:wallet/avatar', upload.single('avatar'), async (req, res) => {
    try {
        ...
        const profile = existing ? { ...existing, avatarUrl: publicUrl } : { username: wallet, avatarUrl: publicUrl };
...
function adminAuth(req, res, next) {
    try {
        ...
        if (!payload || payload.u !== ADMIN_USER) {
...
app.get("/hand/history", (req, res) => {
    const tableId = req.query.tableId || undefined;
    if (tableId) {
...
async function solRpc(method, params) {
    ...
    if (!r.ok)
        throw new Error(`solana rpc http ${r.status}`);
    const js = await r.json();
    if (js.error)
        throw new Error(String(js.error?.message || js.error));
    return js.result;
}
app.get("/sol/eligibility", async (req, res) => {
    try {
        ...
        const ta = await solRpc("getTokenAccountsByOwner", [owner, { mint }, { encoding: "jsonParsed" }]);
        let uiAmountSum = 0;
        let decimals = null;
        try {
        ...
        if (decimals == null) {
            try {
                const sup = await solRpc("getTokenSupply", [mint]);
                if (typeof sup?.value?.decimals === 'number')
...
app.get("/fairness/commit", (req, res) => {
    const tableId = req.query.tableId || undefined;
    if (!tableId) {
        res.status(400).json({ ok: false, error: "missing tableId" });
        return;
    }
...
const broadcastChat = (tableId, message) => {
    broadcast({ type: "chat", payload: { tableId, message, timestamp: Date.now() } });
};
function resolveDisplayName(wallet) {
    if (!wallet)
        return '';
...
;
globalThis.__pgResolveName = (pid) => resolveDisplayName(pid);
// Deterministic PRNG from hex seed (xorshift32 based on seed hash)
function seedToRng(hexSeed) {
    let h = 2166136261 >>> 0;
...
async function loadProfiles() {
    try {
        ...
        Object.entries(obj || {}).forEach(([wallet, p]) => {
            if (p && typeof p.username === 'string')
                profiles.set(wallet, { username: p.username, avatarUrl: p.avatarUrl });
        });
    }
    catch { }
}
...
const botDueAtMsByTable = new Map();
function scheduleBot(tableId, now, minDelay = 400, maxDelay = 900) {
...
function botTick(now) {
    tableEngines.forEach((eng, tableId) => {
        try {
            ...
            if (!a) {
                botDueAtMsByTable.delete(tableId);
                return;
            }
            if (a.actorPlayerId !== BOT_ID) {
                botDueAtMsByTable.delete(tableId);
                return;
            }
            ...
            const can = (x) => (a.legalActions || []).includes(x);
            // Policy
            if (toCall > 0) {
                if (can('call')) {
            ...
            if (pub.street === "showdown") {
                ...
                try {
                    ...
                    const entry = {
                        ...
                        winners: (pub.lastWinners || []).map((w) => ({ ...w, displayName: resolveDisplayName(w.playerId) })),
                        showdownInfo: (pub.showdownInfo || []).map((s) => ({ ...s, displayName: resolveDisplayName(s.playerId) })),
                        pot: pub.pot,
                    };
                    list.push(entry);
        ...
        if (toRequeue.length) {
            toRequeue.forEach(({ tableId, players, winners }) => {
                ...
                if (players.length === 2 && winners.length >= 1) {
                    const [p1, p2] = players;
                    const winner = winners.includes(p1) ? p1 : winners.includes(p2) ? p2 : null;
...
app.post("/admin/timing", adminAuth, (req, res) => {
    const primary = Number(req.query.primaryMs ?? "");
    const bank = Number(req.query.bankMs ?? "");
...
app.post("/admin/hu/setStacks", express.json(), (req, res) => {
    ...
    try {
        ...
        if (!Array.isArray(e.players)) {
            res.status(500).json({ ok: false, error: 'engine players missing' });
            return;
        }
        e.players.forEach((p) => {
            const raw = stacks ? stacks[p.playerId] : undefined;
...
async function loadHuData() {
    try {
        const buf = await fs.readFile(DATA_FILE, 'utf-8');
        const js = JSON.parse(buf || '{}');
        if (js && js.elo && typeof js.elo === 'object') {
            Object.entries(js.elo).forEach(([k, v]) => { if (typeof v === 'number')
                huElo.set(k, v); });
        }
        if (js && js.leaderboard && typeof js.leaderboard === 'object') {
            Object.entries(js.leaderboard).forEach(([k, v]) => {
                if (v && typeof v.wins === 'number' && typeof v.matches === 'number')
                    huLeaderboard.set(k, { wins: v.wins, matches: v.matches });
            });
        }
    }
    catch { }
...
async function loadUsers() {
    try {
        ...
        Object.values(js || {}).forEach((u) => {
            if (u && typeof u.username === 'string' && typeof u.wallet === 'string' && typeof u.passwordHash === 'string') {
                const user = { username: u.username, wallet: u.wallet, passwordHash: u.passwordHash, createdAt: Number(u.createdAt || Date.now()) };
                usersByName.set(user.username, user);
                usersByWallet.set(user.wallet, user);
            }
        });
    }
    catch { }
...
app.post('/auth/register', express.json(), async (req, res) => {
    ...
    if (usersByName.has(username)) {
        res.status(409).json({ ok: false, error: 'username taken' });
        return;
    }
    if (usersByWallet.has(wallet)) {
        res.status(409).json({ ok: false, error: 'wallet already linked' });
        return;
    }
    const u = { username, wallet, passwordHash: hashPassword(password), createdAt: Date.now() };
    usersByName.set(username, u);
    usersByWallet.set(wallet, u);
