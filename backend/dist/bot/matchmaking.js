...
const botToMatch = new Map();
const STARTING_CHIPS = 1000;
const SMALL_BLIND = 10;
const BIG_BLIND = 20;
const ACTION_TIMEOUT_MS = 10000;
// ============== Queue ==============
export function joinQueue(botId) {
...
export function leaveQueue(botId) {
...
export function getQueuePosition(botId) {
...
function sendGameStateToBots(match) {
...
function getActionRequired(match, botId) {
    const hand = match.currentHand;
    const opponentId = botId === match.bot1Id ? match.bot2Id : match.bot1Id;
...
export function handleBotAction(botId, action, amount) {
    ...
    if (!actionReq.legalActions.includes(action)) {
...
function resolveShowdown(match) {
    const hand = match.currentHand;
...
function endHand(match, winnerId, reason) {
    const hand = match.currentHand;
...
function handleTimeout(matchId, botId, handNumber) {
    const match = matches.get(matchId);
    if (!match || match.status !== 'playing')
        return;
    if (!match.currentHand || match.currentHand.handNumber !== handNumber)
        return;
    if (match.currentHand.toAct !== botId)
        return;
...
export function getBotMatch(botId) {
    const matchId = botToMatch.get(botId);
...
export function isInMatch(botId) {
