...
export function evaluateBestFive(cards) {
    if (cards.length < 5)
        throw new Error("need >=5 cards");
    const ranksCount = new Map();
    ...
    const uniqueWithWheel = uniqueRanks.includes(14) ? uniqueRanks.concat([1]) : uniqueRanks;
    function findStraight(ranks) {
        ...
        for (let i = 0; i < ranks.length - 1; i += 1) {
            if (ranks[i] === ranks[i + 1] + 1) {
                streak += 1;
                if (streak >= 5) {
                    return [ranks[i - 3], ranks[i - 2], ranks[i - 1], ranks[i], ranks[i + 1]];
                }
            }
    ...
    if ((triples.length >= 1 && pairs.length >= 2) || triples.length >= 2) {
        ...
        if (pairRank !== undefined) {
            ...
            return { category: HandCategory.FullHouse, kickers: [topTrips, topTrips, topTrips, pairRank, pairRank], bestFive };
        }
    }
    // Flush
    if (flushCards) {
